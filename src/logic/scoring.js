 ...
 *
 * 武士(bushi)・虚無僧(komuso)・薬師(kusushi)の配点は、武士の選択肢Aが問1・問2で+1、
 * 問3で+3(2026/9/20時点)、虚無僧・薬師は各+2で、器用貧乏枠Dは両者に+2(問1のDは
 * 薬師のみ、問2・問3のDは武士+1も追加)。素直に「同点は問3→問1でタイブレーク」を武士にも適用すると、
 * 際どい同点でも武士が勝ててしまい、意図した「レアさ」が成立しない(武士が
 * +1だった当時、総当たりで全27通り中5通り=18.5%まで上振れすることを確認済み)。
 *
 * そのため武士だけは同点勝ちを許さない特別ルールにしている: 武士が結果に
 * なるのは「武士の合計が、他の2タイプの合計をどちらも単独で上回った場合」
 * のみ(2026/9/20時点の配点では、全体で約3.2%で12タイプ中もっともレア)。同点に武士が含まれる場合は
